...
namespace Sample
{
    D3D12RaytracingReSTIR* g_pSample = nullptr;
    D3D12RaytracingReSTIR& instance()
    {
        return *g_pSample;
    }
...
D3D12RaytracingReSTIR::D3D12RaytracingReSTIR(UINT width, UINT height, wstring name) :
...
void D3D12RaytracingReSTIR::OnInit()
{
    UINT flags =
        m_syncInterval > 0
        ? 0
...
D3D12RaytracingReSTIR::~D3D12RaytracingReSTIR()
{
    GameInput::Shutdown();
}

void D3D12RaytracingReSTIR::WriteProfilingResultsToFile()
{
...
void D3D12RaytracingReSTIR::CreateDeviceDependentResources()
{
...
void D3D12RaytracingReSTIR::CreateRaytracingOutputResource()
{
    auto device = m_deviceResources->GetD3DDevice();
...
void D3D12RaytracingReSTIR::CreateDebugResources()
{
    auto device = m_deviceResources->GetD3DDevice();
    auto backbufferFormat = m_deviceResources->GetBackBufferFormat();
...
void D3D12RaytracingReSTIR::CreateAuxilaryDeviceResources()
{
    auto device = m_deviceResources->GetD3DDevice();
...
void D3D12RaytracingReSTIR::CreateDescriptorHeaps()
{
    auto device = m_deviceResources->GetD3DDevice();
...
void D3D12RaytracingReSTIR::OnKeyDown(UINT8 key)
{
...
void D3D12RaytracingReSTIR::OnUpdate()
{
    if (m_isProfiling)
    {
...
void D3D12RaytracingReSTIR::CopyRaytracingOutputToBackbuffer(D3D12_RESOURCE_STATES outRenderTargetState)
{
    auto commandList = m_deviceResources->GetCommandList();
...
void D3D12RaytracingReSTIR::UpdateUI()
{
...
void D3D12RaytracingReSTIR::CreateWindowSizeDependentResources()
{
    auto device = m_deviceResources->GetD3DDevice();
    auto commandQueue = m_deviceResources->GetCommandQueue();
...
void D3D12RaytracingReSTIR::ReleaseWindowSizeDependentResources()
{
    if (m_enableUI)
    {
        m_uiLayer.reset();
...
void D3D12RaytracingReSTIR::ReleaseDeviceDependentResources()
{
    EngineProfiling::ReleaseDevice();
}

void D3D12RaytracingReSTIR::RecreateD3D()
{
...
// Render the scene.
void D3D12RaytracingReSTIR::OnRender()
{
...
void D3D12RaytracingReSTIR::CalculateFrameStats()
{
...
void D3D12RaytracingReSTIR::OnSizeChanged(UINT width, UINT height, bool minimized)
{
    UpdateForSizeChange(width, height);
